...
// per就是person的实例
```
在上面的Person构造函数中，为每一个对象都添加了一个sayName方法，因为其方法是在构造函数内部创建的
...
如果仍然没有则去原型的原型中寻找，直到找到Object的原型
`Person.prototype.a = 123;`向person的原型中添加属性a 
`Person.prototype.SayHello = function(){}`向person的原型中添加sayhello方法

**所以在创建构造函数时，可以将共有的属性和方法添加到prototype中，达到共同使用的目的**

可以使用in来检查对象中是否含有某个属性，如果对象中没有但是原型中有，也会返回true
